...
def pack_bootstrap_authoring_rules(
    *,
    ...
    return (
        ...
        "- calls.dir_generate.system/instructions must author a questionnaire "
        "for the CURRENT typed host only. Do not hard-code one scenario_id, "
        "process area, or variant (for example downstream_process) as if later "
        "queries must reuse it.\n"
...
def component_schema_hints() -> Dict[str, str]:
    ...
    return {
        ...
        "prompt_fragments.yaml": (
            ...
            "one-menu JSON with requirements[] for the CURRENT typed host, not a "
            "dir_menus YAML wrapper and not a locked first-query scenario_id), "
